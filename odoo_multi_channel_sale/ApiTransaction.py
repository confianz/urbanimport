...
class Transaction:
		...
		if hasattr(self.instance,'import_{}'.format(self.channel)):
			...
			try:
				while True:
					...
					# print('data_listdata_list',data_list)
					# print('kwkw',kw)
					...
					if data_list:
						...
						if kw.get('page_size', 0) == 1:
							if locals().get('old_last_id') == kw.get('last_id'):
								break
								# print('locals bcause of pglocalsz')
						...
						if len(data_list) < kw.get('page_size'):
							# print('brak bcause of pgz')
							break
					else:
						_logger.exception(kw.get('message'))
						break
	...
	def export_data(self, object, object_ids, operation='export'):
		...
		if operation == 'export' and hasattr(self.instance,'export_{}'.format(self.channel)):
			...
			# print('mappingsmappingsmappings',METAMAP.get(object).get('local_field'))
			# print('mappingsmappingsmappings',mappings)
			# print('object_idsobject_idsobject_ids',object_ids)
			# print('local_idslocal_ids',local_ids)

...
class PaymentPageWeb(http.Controller):
    ...
    def customer_verification(self, **form):
        ...
        paid_invoice_ids = sale_obj.invoice_ids.filtered(
            lambda r: r.state == 'paid' and r.move_type == 'out_invoice')
        inv_paid_amount = 0
    ...
    def customer_confirmation_invoice(self, token, error=None):
        ...
        invoice_ids = invoice_ids.filtered(lambda r: r.state == 'posted' and r.move_type == 'out_invoice' and r.payment_state != 'paid')
        credit_memo = request.env['account.move'].sudo().search(
            [('partner_id', '=', edi_record.invoice_id.partner_id.id)])
        credit_memo = credit_memo.filtered(lambda r: r.state == 'draft' and r.move_type == 'out_refund')
        if edi_record:
            edi_record.state = 'draft'
            warning = False
            if error:
                warning = request.env['error.box'].sudo().search([('order', '=', edi_record.invoice_id.name)],
                                                                 order='id DESC', limit=1)
                warning = self.check_string(warning.error_message)
                warning = warning.split("-")
                if len(warning) > 1:
                    warning = warning and warning[1]
            if edi_record.invoice_id.state == 'posted' and edi_record.invoice_id.payment_state == 'paid':
                return request.render("authorize_net_integration.PaymentResultView", {'status': 'invoice-paid'})
            elif edi_record.invoice_id.state != 'posted':
                return request.render("authorize_net_integration.PaymentResultView", {'status': 'not_exist'})
            invoice_ids = sorted(invoice_ids, key=lambda a: a.invoice_date_due)
            journal_rec = request.env['account.journal'].sudo().search([('is_authorizenet', '=', True)], limit=1)
            surcharge_percentage = journal_rec and journal_rec.surcharge_customer or 0
            handing_fee = (edi_record.invoice_id.amount_residual * surcharge_percentage) / 100
    ...
    def customer_verification_invoice(self, **form):
        ...
        if invoice_sudo.state == 'posted' and invoice_sudo.payment_state == 'paid':
            return request.render("authorize_net_integration.PaymentResultView", {'status': 'already'})
        ...
        if edi_record.state != 'submitted':
            ...
            for inv in invoice_list:
                invoice_sudo = request.env['account.move'].sudo().browse(int(inv))
                invoice_sudo.write({'amount_gateway': invoice_sudo.amount_gateway + float(amount)})
            # invoice_sudo = request.env['account.move'].sudo().browse(int(invoice_id))
            # invoice_sudo.write({'amount_gateway': invoice_sudo.amount_gateway + invoice_sudo.amount_residual})
            surcharge = 0
            if 'handling_fee' in form:
                surcharge = form['handling_fee']
            transaction_id, error = request.env['account.move'].sudo().aim_transaction_invoice(invoice_list, amount,
                                                                                                  card, cvv, expiry,
                                                                                                  extra, invoice_id,
                                                                                                  message,
                                                                                                  surcharge=float(
            ...
            if transaction_id:
                ...
                invoices = []
                for inv in invoice_list:
                    invoices.append(request.env['account.move'].sudo().browse(inv))
        ...
        elif edi_record.invoice_id.state == 'posted' and edi_record.invoice_id.payment_state == 'paid':
            return request.render("authorize_net_integration.PaymentResultView", {'status': 'already'})
        else:
            edi_record.state = 'expired'
            return request.render("authorize_net_integration.PaymentResultView", {'status': edi_record.state})

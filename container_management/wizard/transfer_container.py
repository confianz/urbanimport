...
class SelectContainerWizard(models.TransientModel):
    ...
    def default_get(self, fields):
        result = super(SelectContainerWizard, self).default_get(fields)
        active_model = self._context.get('active_model')
        if active_model == 'container.container':
            container = self.env['container.container'].browse(self._context.get('active_ids'))
            container_lines = container.mapped('container_lines')
            destination_loc_id = container_lines and container_lines[0].po_id.end_loc_id.id or False
        if active_model == 'stock.picking':
            picking_id = self.env['stock.picking'].browse(self._context.get('active_id'))
            po_id = self.env['purchase.order'].search([('name', '=', picking_id.origin)])
            destination_loc_id = po_id and po_id.end_loc_id.id or False
        result['destination_loc_id'] = destination_loc_id
        return result
    ...
    def transfer_container_to_wh(self):
        ...
        picking_type_id = self.env['stock.picking.type'].search(
            [('code', '=', 'incoming'), ('warehouse_id.code', '=', warehouse)], limit=1)
        if self._context.get('direct_transfer', False):
            picking_id = self.env['stock.picking'].browse(self._context.get('active_id'))
            if picking_id.state != 'done':
                raise UserError(
                    "Products should available in Ocean inorder to transfer to Warehouse or the picking should be done.")
            ...
            picking_id.wh_transfer_done = True
        ...
        for m_line in move_lines.values():
            ...
            cont_line = m_line.get('line_ids')
            container_lines.action_received_in_warehouse()
        containers.create_container_status_note(
            msg="Received in Warehouse:- %s" % (self.destination_loc_id.display_name), user_id=self.env.user)
        picking_ids._update_status()
        picking_ids.filtered(lambda x: x.state == "done").write({'wh_transfer_done':True})
        return True

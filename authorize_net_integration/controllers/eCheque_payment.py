    ...
    def cheque_payment(self, **form):
        ...
        if 'order' in form:
            ...
            draft_invoice_ids = sale_obj.invoice_ids.filtered(
                lambda r: r.state not in ('paid', 'cancel') and r.move_type == 'out_invoice')
            inv_draft_amount = 0
        ...
        if payment_token and not invoice_list:
            if sale_obj and payment_token and payment_token.state not in (
                    'paid', 'expired'):
                # payment_token.invoice_id.write({'amount_gateway': payment_token.invoice_id.residual})
        ...
        # call from multi select invoice payment view
            ...
            if extra:
                extra = float(extra)
            transaction_id, error = request.env['account.move'].sudo().aim_transaction_invoice(

...
class make_payment(models.TransientModel):
    ...
    def authorize_transaction(self):
        ...
        if self.is_correction:
            try:
                authorize_obj.void_payment(profile_id, active_brw.payment_profile_id, active_brw.transaction_id)
            except:
                pass
        ...
        journal_rec = self.env['account.journal'].sudo().search([('is_authorizenet', '=', True)], limit=1)
        surcharge_percentage = journal_rec and journal_rec.surcharge_user or 0
        handling_fee = (InvoiceRec.amount_residual * surcharge_percentage) / 100
        amount = InvoiceRec.amount_residual + handling_fee
        payment = InvoiceRec.with_context({'payment_amount': amount}).register_card_payments()
        # print('------------------------------------',payment)

        transaction_id = authorize_obj.authorize_payment(profile_id, payment_id, InvoiceRec.amount_residual,
                                                         InvoiceRec.name)
        payment.write(
            {'transaction_id': transaction_id, 'payment_id': payment_id})
        self.env['authorizenet.api'].capture_payment(profile_id, payment_id, transaction_id, InvoiceRec.amount_residual)
        InvoiceRec.write(
            {'transaction_id': transaction_id, 'payment_profile_id': payment_id, 'amount_gateway': InvoiceRec.amount_residual,

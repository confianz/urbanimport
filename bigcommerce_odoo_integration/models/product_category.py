...
class ProductCategory(models.Model):
    ...
    def category_request_data(self, category_id):
        ...
        return {
            "parent_id": int(
                category_id and category_id.parent_id and category_id.parent_id.bigcommerce_product_category_id) or 0,
    ...
    def odoo_to_bigcommerce_export_product_categories(self, warehouse_id=False, bigcommerce_store_ids=False):
        for bigcommerce_store_id in bigcommerce_store_ids:
            ...
            category_operation_id = self.create_bigcommerce_operation('product_category', 'export',
                                                                      bigcommerce_store_id, 'Processing...',
                                                                      warehouse_id)
            self._cr.commit()
            try:
                category_ids = self.search(
                    [('bigcommerce_product_category_id', '=', False), ('is_exported_to_bigcommerce', '=', True)])
                ...
                for category_id in category_ids:
                    ...
                    response_data = bigcommerce_store_id.send_request_from_odoo_to_bigcommerce(category_request_data,
                                                                                               api_operation)
                    ...
                    if response_data.status_code in [200, 201]:
                        ...
                        if response_data.get('data') and response_data.get('data').get("id"):
                            ...
                            process_message = "%s : %s Product Category Exported Successfully." % (
                            bigcommerce_category_id, category_id.name)
                            self.create_bigcommerce_operation_detail('product_category', 'export',
                                                                     category_request_data, response_data,
                        ...
                        self.create_bigcommerce_operation_detail('product_category', 'export', category_request_data,
                                                                 error_msg, category_operation_id, warehouse_id, True,
                                                                 error_msg)
                    self._cr.commit()
            except Exception as e:
                category_process_message = "Process Is Not Completed Yet!  {}".format(e)
                self.create_bigcommerce_operation_detail('product_category', 'export', category_request_data,
                                                         response_data, category_operation_id, warehouse_id, True,
    ...
    def bigcommerce_to_odoo_import_product_categories(self, warehouse_id=False, bigcommerce_store_ids=False):
        for bigcommerce_store_id in bigcommerce_store_ids:
            ...
            category_process_message = "Process Completed Successfully!"
            category_operation_id = self.create_bigcommerce_operation('product_category', 'import',
                                                                      bigcommerce_store_id, 'Processing...',
                                                                      warehouse_id)
            self._cr.commit()
            try:
                    ...
                    if total_pages > 1:
                        while (total_pages != 0):
                            try:
                                ...
                                self.create_bigcommerce_operation_detail('product_category', 'import',
                                                                         page_response_data,
                                                                         category_process_message,
                                                                         category_operation_id, warehouse_id, True,
                                                                         process_message)

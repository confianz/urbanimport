...
class AuthorizenetApi(models.Model):
    ...
    def authorize_capture_transaction_aim(self, amount, card, cvv, expiry, invoice):
        ...
        auth_data = """<?xml version="1.0" encoding="utf-8"?>
                    ...
                    """ % (
            auth_header, round(float(amount), 2), card, expiry, cvv, customer_details.name, id, email, first_name,
            street,
    ...
    def authorize_capture_cheque_transaction_aim(self, amount, invoice, account_name, routing_number, account_number,
        ...
        invoice_details = self.env['account.move'].search([('id', '=', int(invoice))])
        ...
        auth_data = """<?xml version="1.0" encoding="UTF-8"?>
            ...
            eCheque_type, bank_name, invoice_details.name, invoice_details.name,
            id, invoice_details.name, round(float(amount), 2), invoice_details.partner_id.id, first_name, first_name,

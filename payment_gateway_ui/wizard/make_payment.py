...
class make_payment(models.TransientModel):
    ...
    def gateway_transaction(self):
        self.ensure_one()
        # print('self._context######', self._context,'\t\t\t%s' % self.payment_id)
        # print('self._context-----', self.env.context)
        # input()
        # import time
        # time.sleep(4)
        # return True
        ...
        if self.env.context.get('active_model',
                                '') == 'account.move':
            if active_brw.amount_residual != active_brw.amount_total or active_brw.amount_gateway:
                raise UserError(
        ...
        # print('method_call', method_call,'\t\t\t%s' % self.payment_id)
        if hasattr(self, method_call):
            # print('inside hook','\t\t\t%s' % self.payment_id)
    ...
    exp_year = fields.Selection(
        [(str(num), str(num)) for num in range(datetime.now().year, (datetime.now().year) + 11)],

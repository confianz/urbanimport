...
class DeliveryCarrier(models.Model):
    ...
    @api.model
    def check_address_validation(self, address):
        res = False
        result = self
        environment = True if not result.prod_environment else False
        con = FedexConfig(key=result.fedex_developer_key,
                          password=result.fedex_developer_password,
                          account_number=result.fedex_account_number,
        ...
        address1.Address.PostalCode = address.get('zip')
        address1.Address.CountryCode = address.get('country_code')
        connection.add_address(address1)

        try:
            connection.send_request()
        except Exception as  e:
            raise UserError(_('Address validation Error \n %s' % e))

        if connection.response:
            logging.error('RESPONSE')
            logging.error(connection.response)
            if connection.response.HighestSeverity == "SUCCESS":
                for result in connection.response.AddressResults:
                    if 'Classification' in result:
                        return result['Classification']
        return res

    def fedex_validate_address(self, partner=False):
        partner_address = partner._get_address()
        Response = self.check_address_validation(
            partner_address)
        return Response

    def fedex_rate_shipment_custom(self, order, warehouse):
        ...
        srm.set_shipper(order.company_id.partner_id, warehouse.partner_id)
        srm.set_recipient(order.partner_shipping_id)
        if self.fedex_service_type == 'GROUND_HOME_DELIVERY':
            srm.RequestedShipment.Recipient.Address.Residential = True
        ...
        return {'success': True,
                'price': price,
                'time': request['DeliveryTimestamp'],
                'error_message': False,

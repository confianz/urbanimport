        ...
        if response.status_code in [200,201]:
            ...
            for data in response.get('data'):
                ...
                if data.get('is_thumbnail'):
                    product_id.image_1920 = image_data
                    product_id.bc_product_image_id = image_id
                else:
                    ecomm_product_image_id = self.env['product.image'].search(
                        [('bc_product_image_id', '=', image_id)])
                    if not ecomm_product_image_id:
                        ecomm_product_image.update({
                            'bc_product_image_id': image_id,
                            'name': description if description else product_id.name
                        })
                        ecomm_product_image_id = self.env['product.image'].create(ecomm_product_image)
                        ecomm_product_image_id.write(
                            {'name': ecomm_product_image_id.name + ":" + str(ecomm_product_image_id.id)})
                        _logger.info("Successfully Image Added{0}".format(product_id.id))
                    else:
                        ecomm_product_image.update({
                            'bc_product_image_id': image_id,
                            'name': (description if description else product_id.name) + ":" + str(
                                ecomm_product_image_id.id)
                        })
                        ecomm_product_image_id.write(ecomm_product_image)
